...
(function(){
  ...
  // Tạo blob path hữu cơ (mềm mại) theo seed để ổn định theo từ
  function makeBlobPath(w, h, seed){
    const cx = w/2, cy = h/2;
    const rx = (w - 64)/2, ry = (h - 48)/2;
    const n = 8; // số điểm quanh vòng
    const phase = (seed % 360) * Math.PI/180;
    const amp = 0.08 + (seed % 40)/1000; // biên độ biến đổi nhẹ
    const pts = [];
    for (let i=0; i<n; i++){
      const a = 2*Math.PI * i/n;
      const rFactor = 1 + amp * Math.sin(a*2 + phase);
      const x = cx + rx * rFactor * Math.cos(a);
      const y = cy + ry * rFactor * Math.sin(a);
      pts.push({x,y});
    }
    // Tạo đường cong mượt dạng chuỗi Q (midpoint smoothing)
