...
window.ReadingModule = {
    ...
    for (let i=0;i<parts.length;i++){
      ...
      if (/^[.!?…]+$/.test(p)) {
        ...
        for (let k=0;k<words.length;k++){
          ...
          span.style.transition = 'background 0.2s, box-shadow 0.2s, outline 0.2s, border-bottom-color 0.2s';
  ...
  renderPassageTokens(){
    ...
    Object.keys(this.errors).forEach(k=>{
      ...
      if (el) { el.style.outline = '3px solid var(--danger)'; el.setAttribute('aria-pressed','true'); }
    });
    this.applyFocusMask();
  },
  ...
  clearMarks(){
    this.errors = {};
    for (const el of this.tokenElems) { el.style.outline = 'none'; el && el.setAttribute('aria-pressed','false'); }
    const sW = document.getElementById('statWCPM'); if (sW) sW.textContent='—';
    const sA = document.getElementById('statAcc'); if (sA) sA.textContent='—';
  },
  onTokenClick(i, el){
    if (this.markModeState!=='error') return;
    if (this.errors[i]) { delete this.errors[i]; el.style.outline = 'none'; el.setAttribute('aria-pressed','false'); }
    else { this.errors[i] = { type: 'other' }; el.style.outline = '3px solid var(--danger)'; el.setAttribute('aria-pressed','true'); }
  ...
  setErrType(t){
    ...
    const el = this.tokenElems[this._errTarget]; if (el) { el.style.outline = '3px solid var(--danger)'; el.setAttribute('aria-pressed','true'); }
    const em = document.getElementById('errorMenu');
